...
*     May 10, 1996
*
*     This routine contains a sample call to PSSYEV.
...
*     .. Parameters ..
      INTEGER            LWORK, MAXN
      REAL               ZERO
      PARAMETER          ( LWORK = 500, MAXN = 100, ZERO = 0.0E+0 )
      INTEGER            LDA
      REAL               MONE
      INTEGER            MAXPROCS
...
*     .. Local Arrays ..
      ...
      REAL               A( LDA, LDA ), W( MAXN ), WORK( LWORK ),
     $                   Z( LDA, LDA ), Zref(2,2)
*     ..
*     .. External Subroutines ..
      EXTERNAL           BLACS_EXIT, BLACS_GET, BLACS_GRIDEXIT,
     $                   BLACS_GRIDINFO, BLACS_GRIDINIT, BLACS_PINFO,
     $                   BLACS_SETUP, DESCINIT, PSLAMODHILB, PSLAPRNT,
     $                   PSSYEV
*     ..
...
*
      ...
      Zref = reshape([-0.0485377931880383862,-0.282485135303397583,
     &  0.495193043045525749, -0.298844131980118444],
     & shape(Zref))
*
*
...
*
      IF( MYROW.EQ.-1 ) CALL BLACS_EXIT( 0 )
*
*
...
!     END IF
      ...
      IF( MYROW.EQ.0 .AND. MYCOL.EQ.0 ) then
        if(any(abs(Z(:N,:N)-Zref) > 0.00001)) then
            error stop "too high error"
        endif
      endif
